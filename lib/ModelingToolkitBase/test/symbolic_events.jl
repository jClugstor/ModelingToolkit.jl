...
@testset "ImperativeAffect constructors" begin
    ...
    m = ModelingToolkitBase.ImperativeAffect(fmfa)
    ...
    m = ModelingToolkitBase.ImperativeAffect(fmfa, (; x))
    @test m isa ModelingToolkitBase.ImperativeAffect
    @test m.f == fmfa
    @test isequal(m.obs, [])
    @test m.obs_syms == []
    ...
    m = ModelingToolkitBase.ImperativeAffect(fmfa, (; y = x))
    @test m isa ModelingToolkitBase.ImperativeAffect
    @test m.f == fmfa
    @test isequal(m.obs, [])
    @test m.obs_syms == []
    ...
    m = ModelingToolkitBase.ImperativeAffect(fmfa; observed = (; y = x))
    @test m isa ModelingToolkitBase.ImperativeAffect
    @test m.f == fmfa
    ...
    m = ModelingToolkitBase.ImperativeAffect(fmfa; modified = (; x))
    @test m isa ModelingToolkitBase.ImperativeAffect
    @test m.f == fmfa
    @test isequal(m.obs, [])
    @test m.obs_syms == []
    ...
    m = ModelingToolkitBase.ImperativeAffect(fmfa; modified = (; y = x))
    @test m isa ModelingToolkitBase.ImperativeAffect
    @test m.f == fmfa
    @test isequal(m.obs, [])
    @test m.obs_syms == []
    ...
    m = ModelingToolkitBase.ImperativeAffect(fmfa, (; x), (; x))
    @test m isa ModelingToolkitBase.ImperativeAffect
    @test m.f == fmfa
    @test isequal(m.obs, [x])
    @test m.obs_syms == [:x]
    ...
    m = ModelingToolkitBase.ImperativeAffect(fmfa, (; y = x), (; y = x))
    @test m isa ModelingToolkitBase.ImperativeAffect
    @test m.f == fmfa
    @test isequal(m.obs, [x])
    @test m.obs_syms == [:y]
    ...
    m = ModelingToolkitBase.ImperativeAffect(fmfa, (; x), (; x), 3)
    @test m isa ModelingToolkitBase.ImperativeAffect
    @test m.f == fmfa
    @test isequal(m.obs, [x])
    @test m.obs_syms == [:x]
...
@testset "Condition Compilation" begin
    ...
    @test isequal(equations(ModelingToolkitBase.continuous_events(sys2)[1])[], x ~ 2)
    @test isequal(equations(ModelingToolkitBase.continuous_events(sys2)[2])[], sys.x ~ 1)
    ...
    @test cb isa ModelingToolkitBase.DiffEqCallbacks.VectorContinuousCallback
...
@testset "Namespacing" begin
    ...
    sol = solve(prob, @isdefined(ModelingToolkit) ? Tsit5() : Rodas5P(), saveat = 0.1)
...
@testset "Initialization" begin
    ...
    inited = false
    finaled = false
    cb3 = ModelingToolkitBase.SymbolicDiscreteCallback(1.0, f, initialize = a, finalize = b)
    @mtkcompile sys = System(D(x) ~ -1, t, [x], []; discrete_events = [cb3])
    prob = ODEProblem(sys, [x => 1.0], (0.0, 2))
    sol = solve(prob, Tsit5())
    @test seen == true
...
if @isdefined(ModelingToolkit)
    @testset "Symbolic affects are compiled in `complete`" begin
        ...
        @test scc.affect isa ModelingToolkitBase.SymbolicAffect
...
@testset "Algebraic equation with input variable in symbolic affect" begin
    ...
    dev = ModelingToolkitBase.SymbolicDiscreteCallback(1.0, [y ~ Pre(y) + 1])
...
@testset "Issue#3990: Scalarized array passed to `discrete_parameters` of symbolic affect" begin
    ...
    v_event = ModelingToolkitBase.SymbolicDiscreteCallback(
        1.0,
        [v ~ -Pre(v)],
        discrete_parameters = [v]
    )

    M_event = ModelingToolkitBase.SymbolicDiscreteCallback(
        1.0,
        [M ~ -Pre(M)],
