...
@testset "Constants" begin
    ...
    @test symtype(ModelingToolkitBase.getdefault(pi_model.p)) <: Real
...
@testset "Parameters and Structural parameters in various modes" begin
    ...
    @test hasmetadata(model.l, ModelingToolkitBase.VariableDescription)
    ...
    @test value(get_initial_conditions(model)[model.n]) == 1.0
    @test value(get_initial_conditions(model)[model.n2]) == 5
...
@testset "Component array" begin
    ...
    @test nameof.(ModelingToolkitBase.get_systems(component)) == [
        :comprehension_1,
        :comprehension_2,
        :written_out_for_1,
        :written_out_for_2,
...
@testset "Argument order" begin
    ...
    defs = ModelingToolkitBase.initial_conditions(ordermodel)
...
@testset "Nonnumeric parameters" begin
    ...
    @test value(ModelingToolkitBase.initial_conditions(sys)[@nonamespace sys.p]) == CustomStruct()
end
...
@testset "Constraints, costs, consolidate" begin
    ...
    costs = ModelingToolkitBase.get_costs(ex)
    constrs = ModelingToolkitBase.get_constraints(ex)
    ...
    @test ModelingToolkitBase.get_consolidate(ex)([1, 2], [3, 4]) ≈ 8 + log(2)
...
@testset "Model Level Metadata" begin
    ...
    @show getmetadata(t.val, ModelingToolkitBase.VariableUnit, nothing)
    struct Author end
    ...
    @test ModelingToolkitBase.getmetadata(test_model, Author, nothing) == "Test Author"
    @test ModelingToolkitBase.getmetadata(test_model, MyVersion, nothing) == "1.0.0"
    @test ModelingToolkitBase.getmetadata(test_model, UnknownMetaKey, nothing) === nothing
    @test ModelingToolkitBase.getmetadata(test_model, MyBool, nothing) === false
    @test ModelingToolkitBase.getmetadata(test_model, NewInt, nothing) === 1
end
...
@testset "Pass parameters of higher level models as structural parameters" begin
    let D = ModelingToolkitBase.D_nounits, t = ModelingToolkitBase.t_nounits
        ...
        if @isdefined(ModelingToolkit)
            ...
            p = MTKParameters(sys, merge(initial_conditions(sys), bindings(sys)))
            u = [0.5 for i in 1:2]
            du = zeros(2)
            ...
            symnames = string.(ModelingToolkitBase.getname.(variable_symbols(sys)))
...
@testset "Scope of defaults in the systems generated by @named" begin
