...
ModelingToolkitBase.toexpr.(eqs)[1]
...
function test_diffeq_inference(name, sys, iv, dvs, ps)
    return @testset "System construction: $name" begin
...
du = @SArray zeros(3)
u = SVector(1:3...)
p = ModelingToolkitBase.MTKParameters(de, SVector{3}([σ, ρ, β] .=> 4.0:6.0))
...
@test f.sparsity == ModelingToolkitBase.jacobian_sparsity(de)
...
de = complete(de)
ModelingToolkitBase.calculate_tgrad(de)

tgrad_oop, tgrad_iip = eval.(ModelingToolkitBase.generate_tgrad(de))
...
@named de = System(eqs, t)
jac = calculate_jacobian(de)
@test ModelingToolkitBase.jacobian_sparsity(de).colptr == sparse(Num.(jac)).colptr
@test ModelingToolkitBase.jacobian_sparsity(de).rowval == sparse(Num.(jac)).rowval
...
prob12 = ODEProblem(sys, [u0; [k₁ => 0.04, k₂ => 3.0e7, k₃ => 1.0e4]], tspan)
...
for prob in [prob4, prob5]
    ...
    sol = solve(prob, IDA())
    @test all(x -> ≈(sum(x), 1.0, atol = 1.0e-12), sol.u)
end
...
@test ModelingToolkitBase.isautonomous(sys)
...
@named sys = System(eqs, t)
@test isequal(ModelingToolkitBase.get_iv(sys), t)
...
@test_throws ArgumentError ModelingToolkitBase.System(eqs, t, vars, pars, name = :foo)
...
@test ModelingToolkitBase.isvariable(Symbolics.unwrap(x[1]))
prob = ODEProblem(sys, [], (0, 1.0))
sol = solve(prob, Tsit5())
@test sol[2x[1] + 3x[3] + norm(x)] ≈
    2sol[x[1]] + 3sol[x[3]] + sol[norm(x)]
@test sol[x .+ [y, 2y, 3y]] ≈ map(
    (x...) -> [x...],
...
function submodel(; name)
    ...
    @parameters A[1:5]
    A = collect(A)
    return System(D(y) ~ sum(A) * y, t; name = name)
end
...
let
    vars = @variables x y
    arr = ModelingToolkitBase.varmap_to_vars(
        Dict([x => 0.0, y => [0.0, 1.0]]), vars; use_union = true
    ) #error
    sol = Union{Float64, Vector{Float64}}[
        0.0, [
            0.0, 1.0,
        ],
    ]
...
let
    function sys1(; name)
        ...
        return System([D(x) ~ dx], t, vars, []; name, initial_conditions = [D(x) => x])
    end

    function sys2(; name)
        @named s1 = sys1()

        return System(Equation[], t, [], []; systems = [s1], name)
    end
...
Set(unknowns(new_sys2)) == Set(
    [
        new_sys2.x1, new_sys2.sys1.x1,
...
@test isequal(ModelingToolkitBase.bindings(model)[model.constant.k], model.k2[1])
...
function RealExpression(; name, y)
    ...
    return sys = System(eqns, t, vars, []; name)
end
...
function RealExpressionSystem(; name)
    ...
    systems = [e1, e2]
    return System(Equation[], t, Iterators.flatten(vars), []; systems, name)
end
...
@test value(ModelingToolkitBase.guesses(outer)[sys.x]) == 1.0
outer = mtkcompile(outer)
@test value(ModelingToolkitBase.get_guesses(outer)[sys.x]) == 1.0
...
@testset "Inplace observed functions" begin
    ...
    ps = ModelingToolkitBase.MTKParameters(sys, [P => 2.0])
    buffer = zeros(3)
...
@testset "Extend systems with a field that can be nothing" begin
    ...
    n_core_metadata = length(ModelingToolkitBase.get_metadata(A1))
    @test length(ModelingToolkitBase.get_metadata(extend(A1, B1))) == n_core_metadata
    meta = ModelingToolkitBase.get_metadata(extend(A1, B2))
    @test length(meta) == n_core_metadata + 1
    @test meta[String] == 2
    meta = ModelingToolkitBase.get_metadata(extend(A2, B1))
    @test length(meta) == n_core_metadata + 1
    @test meta[Int] == 1
    meta = ModelingToolkitBase.get_metadata(extend(A2, B2))
...
@testset "Scalarized parameters in array functions" begin
    ...
    fn1, = ModelingToolkitBase.generate_rhs(sys1; expression = Val{false})
    ps = MTKParameters(sys1, [x => 2ones(2), p => 3ones(2, 2)])
    @test_nowarn fn1(ones(4), ps, 4.0)
    sys2 = mtkcompile(sys, inputs = [x...], outputs = [], split = false)
    fn2, = ModelingToolkitBase.generate_rhs(sys2; expression = Val{false})
    ps = zeros(8)
...
@testset "Substituting preserves parameter dependencies, initial conditions, guesses" begin
    ...
    @test length(ModelingToolkitBase.initial_conditions(sys2)) == 1
    @test value(ModelingToolkitBase.initial_conditions(sys2)[p3]) == 1.0
    @test length(ModelingToolkitBase.guesses(sys2)) == 2
    @test value(ModelingToolkitBase.guesses(sys2)[p3]) == 2.0
...
@testset "Substituting with nested systems" begin
    ...
    @test length(ModelingToolkitBase.initial_conditions(sys2)) == 2
    @test isequal(ModelingToolkitBase.bindings(sys2)[p5], 3p3)
    @test length(ModelingToolkitBase.guesses(sys2)) == 3
    @test value(ModelingToolkitBase.guesses(sys2)[p5]) == 10.0
end
...
@testset "Observed with inputs" begin
    ...
    @test_nowarn ModelingToolkitBase.build_explicit_observed_function(sys1, u; inputs = [x...])
...
@testset "`complete` with `split = false` removes the index cache" begin
    ...
    @test ModelingToolkitBase.get_index_cache(sys) !== nothing
    sys2 = complete(sys; split = false)
    @test ModelingToolkitBase.get_index_cache(sys2) === nothing
end
...
@testset "DAEProblem with array parameters" begin
    ...
    @test sol[x] ≈ sol[y^2 - sum(p)] atol = 1.0e-5
end
...
@testset "Validate input types" begin
    ...
    @variables X(t)::Int64
    eq = D(X) ~ p - d * X
    @test_throws ModelingToolkitBase.ContinuousOperatorDiscreteArgumentError @mtkcompile osys = System(
        [eq], t
    )
...
@testset "`build_explicit_observed_function` with `expression = true` returns `Expr`" begin
    ...
    obsfn_expr = ModelingToolkitBase.build_explicit_observed_function(
        sys, 2x + 1, expression = true
    )
    @test obsfn_expr isa Expr
    obsfn_expr_oop,
        obsfn_expr_iip = ModelingToolkitBase.build_explicit_observed_function(
...
@testset "`@named` always wraps in `ParentScope`" begin
    function SysA(; name, var1)
        @variables x(t)
        scope = ModelingToolkitBase.getmetadata(unwrap(var1), ModelingToolkitBase.SymScope, nothing)
...
@testset "`ProblemTypeCtx`" begin
    @variables x(t)
    @mtkcompile sys = System(
        [D(x) ~ x], t; metadata = [ModelingToolkitBase.ProblemTypeCtx => "A"]
    )
...
@testset "`substitute` retains events and metadata" begin
    ...
    @named sys = System(
        eq, t, [x, p], [], discrete_events = [event],
    ...
    @test length(ModelingToolkitBase.get_discrete_events(sys2)) == 1
    @test length(ModelingToolkitBase.get_continuous_events(sys)) == 1
    @test length(ModelingToolkitBase.get_continuous_events(sys2)) == 1
